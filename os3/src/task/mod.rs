...
impl TaskManager {
    ...
    fn run_first_task(&self) -> ! {
        ...
        task0.update_first_run_time(get_time_us()/1000);
    ...
    fn run_next_task(&self) {
        if let Some(next) = self.find_next_task() {
            ...
            if inner.tasks[next].get_first_run_time()==0 {
                // 说明是第一次运行
                inner.tasks[next].update_first_run_time(get_time_us()/1000);
            }
    ...
    /// update the number of the syscall for the current tasks
